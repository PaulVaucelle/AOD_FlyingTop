...
//$$$$  if ( tree_passesHTFilter ) {
    ...
    for (size_t iTrack = 0; iTrack<trackRefs.size(); ++iTrack) {
      ...
      int firstHit = -1;
      //----------------------------------//
        const pat::PackedCandidate& pfCand = pc->at(iTrack);
        pat::PackedCandidateRef pcref = pat::PackedCandidateRef(pcs, iTrack);
 
        //  std::cout<<"patcand dxy PV"<<pcref->dxy(PV.position()) <<std::endl;
        //   std::cout<<"patcand dxy"<<pcref->dxy() <<"patcand dxyError"<<pcref->dxyError() <<std::endl;
        float MINIAOD_dxy = -10000;
        float MINIAOD_dxyError = -10000;
        float MINIAOD_pt =-10000;
        float MINIAOD_eta =-10000;
        float MINIAOD_phi =-10000;
        float MINIAOD_NChi2 =-10000;
        float MINIAOD_x =-10000;
        float MINIAOD_y =-10000;
        float MINIAOD_z =-10000;
        float MINIAOD_dz  =-10000;
        float MINIAOD_dzError = -10000;

      if (pcref->hasTrackDetails())
      {
               const reco::Track *trackPcPtr = pcref->bestTrack();//dxy change?
        if( !trackPcPtr ) continue;
                     count3_mini=count3_mini+1;

      const reco::Track& tk = *trackPcPtr;
     
          MINIAOD_dxy = tk.dxy(PV.position());
          MINIAOD_dxyError = tk.dxyError();
          MINIAOD_pt =tk.pt();
         MINIAOD_eta =tk.eta();
         MINIAOD_phi =tk.phi();
        //  MINIAOD_NChi2 =tk.normalizedChi2_();
         MINIAOD_x =tk.vx();
         MINIAOD_y =tk.vy();
         MINIAOD_z =tk.vz();
         MINIAOD_dz  =tk.dz();
         MINIAOD_dzError = tk.dzError();
          // if (ddxy<0.001  && ddxyError<0.001){std::cout<<"RecoToPfcand matched : "<<ipc<< std::endl;}}
      }
      //pas ordoné de la mêem manière ou pas la collection? ou j'en ai aucne id
          tree_track_ddxy.push_back(abs(itTrack->dxy(PV.position())-MINIAOD_dxy));
          tree_track_ddxyError.push_back(abs(itTrack->dxyError(PV.position(),PV.covariance())-MINIAOD_dxyError));
                    tree_track_dpt.push_back(itTrack->pt()-MINIAOD_pt);
          tree_track_deta.push_back(itTrack->eta()-MINIAOD_eta);
                    tree_track_dphi.push_back(itTrack->phi()-MINIAOD_phi);
          tree_track_dNChi2.push_back(itTrack->normalizedChi2()-MINIAOD_NChi2);
                    tree_track_dvx.push_back(itTrack->vx()-MINIAOD_x);
          tree_track_dvy.push_back(itTrack->vy()-MINIAOD_y);
                    tree_track_dvz.push_back(itTrack->vz()-MINIAOD_z);
          tree_track_ddz.push_back(itTrack->dz()-MINIAOD_dz);
                    tree_track_ddzError.push_back(abs(itTrack->dzError()-MINIAOD_dzError));

          //----------------------------------//
  //     if ( pfCand.charge() != itTrack->charge() ) continue;
  //     if ( pfCand.pt() < 1. ) continue;
  //       float dpt  = abs( itTrack->pt() / pfCand.pt() - 1. );
  //       float deta = abs( itTrack->eta() - pfCand.eta() );
  //       float dphi = abs( Deltaphi( itTrack->phi(), pfCand.phi() ) );
	// if ( dpt < 0.01 && deta < 0.01 && dphi < 0.01 ) {
	//   firstHit = pfCand.firstHit();
	//   break;
	// }
...
//$$$$      
      // extrapolate from track origin to its first hits
      float rho = itTrack->pt() * 100 / 0.3 / 3.8;

      // using true information
      float dx = -100., dy = -100., dz = -100.;
      float dphi = -4.;
      float R = TMath::Sqrt( itTrack->innerPosition().X()*itTrack->innerPosition().X()
                           + itTrack->innerPosition().Y()*itTrack->innerPosition().Y() );
      float a = itTrack->charge() * R / rho / 2.;
      float phi = itTrack->phi();
      float s = sin( phi );
      float c = cos( phi );
      if ( a < 1. ) {
        float aa = TMath::Sqrt(1-a*a);
        float x1 =  a*R*s + aa*R*c;
        float y1 = -a*R*c + aa*R*s;
	float phi1 = TMath::ATan2( y1 , x1 );
        float x2 =  a*R*s - aa*R*c;
        float y2 = -a*R*c - aa*R*s;
	float phi2 = TMath::ATan2( y2 , x2 );
  	float dphi1 = Deltaphi( phi1 , phi );
  	float dphi2 = Deltaphi( phi2 , phi );
	if ( abs(dphi2) < abs(dphi1) ) {
	  x1 = x2;
	  y1 = y2;
	  phi1 = phi2;
	}
	dx = x1 - itTrack->innerPosition().X(); 
	dy = y1 - itTrack->innerPosition().Y();
	dphi =  Deltaphi( phi1 , itTrack->innerPosition().phi() );
      } 
      float eta = itTrack->eta();
      float theta = 2. * TMath::ATan(exp(-eta));
      float tgtet = TMath::Tan(theta);
      if ( tgtet != 0. ) {
        float z1 = R / tgtet + itTrack->vz();
	dz = z1 - itTrack->innerPosition().Z();
      } 
      tree_track_extraTrue_dx.push_back(   dx);
      tree_track_extraTrue_dy.push_back(   dy);
      tree_track_extraTrue_dz.push_back(   dz);
      tree_track_extraTrue_dphi.push_back( dphi);

      // using layer number in HitPattern
      dx = -100.; dy = -100.; dz = -100.; dphi = -4.;
      if ( firstHit > 0 ) {
        if ( Central[firstHit] ) {
	  R = Layer[firstHit];
	}
	else {
	  R = (Layer[firstHit] - itTrack->vz()) * tgtet;
	  dz = Layer[firstHit] - itTrack->innerPosition().Z();
	  if ( eta < 0. ) dz = -Layer[firstHit] - itTrack->innerPosition().Z();
	}
        float a = itTrack->charge() * R / rho / 2.;
        float phi = itTrack->phi();
        float s = sin( phi );
	  ...
	  dy = y1 - itTrack->innerPosition().Y();
	  dphi =  Deltaphi( phi1 , itTrack->innerPosition().phi() );
          if ( Central[firstHit] && tgtet != 0. ) {
            float z1 = R / tgtet + itTrack->vz();
	    dz = z1 - itTrack->innerPosition().Z();
          } 
        } 
      } 
      tree_track_fromLayer_dx.push_back(   dx);
      tree_track_fromLayer_dy.push_back(   dy);
      tree_track_fromLayer_dz.push_back(   dz);
      tree_track_fromLayer_dphi.push_back( dphi);
//$$$$      
...
// std::cout<<"axes is ok"<<std::endl;
    ...
    // std::cout<<"comparison is ok"<<std::endl;
...
//$$
    ...
    // std::cout<<"slection is ok"<<std::endl;
    if ( showlog ) {
...
//$$
    ...
    // std::cout<<"vertex fitting is ok"<<std::endl;
//$$
